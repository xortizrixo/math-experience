...
Deno.serve(async (req: Request) => {
  try {
    ...
    // product identifier; the authoritative price lives in this server-side config map.
    const PRODUCTS: Record<string, { name: string; price: string; currency: string }> = {
      unlock_all: { name: "Unlock All Levels", price: "4.99", currency: "USD" },
    };
    ...
    // Fixed-entitlement product: always quantity 1, ignore any client-sent quantity.
    const quantity = 1;
